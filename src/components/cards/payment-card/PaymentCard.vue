				...
				<div class="flex items-center px-4 py-2">
					<div class="flex flex-col my-4 ml-2">
						<div class="flex flex-col">
							<slot name="details"><p class="text-sm text-purple-500 font-medium antialiased leading-none -mt-2">Connect an account to receive money</p></slot>
							<h4 class="text-header-four text-purple-500 font-bold mt-2">{{ title }}</h4>
						</div>
					</div>
				</div>
			</div>
...
export default {
	...
	props: {
		...
		img: {
			type: String,
			default: './assets/svg/other/add-circle-outline-purple.svg'
		}
	}
}
</script>
